...
public class DisableSubCommand extends AbsWorldRelatedSubCommand {
    ...
    public boolean checkPermission(Player player) {
        return player.hasPermission("me.kelgors.me.kelgors.me.kelgors.ubackup.disable") || player.hasPermission("me.kelgors.me.kelgors.me.kelgors.ubackup.*");
    }
    ...
    public boolean execute(CommandSender sender, Command command, String commandName, String[] args) {
        ...
        mPlugin.getConfig().set(String.format("worlds.%s.enabled", worldName), false);
        mPlugin.saveConfig();
        config.enabled = false;
        sender.sendMessage(String.format("The world %s has been disabled", ChatUtils.colorized(ChatColor.BLUE, worldName)));
        return true;
    }
